...
export default async function sendTwoFactorAuthCode(data: any, context: CallableContext): Promise<{ checkpoint: any, message: string } | null> {
  ...
  await instagram.account.twoFactorLogin({
    verificationCode: securityCode,
    twoFactorIdentifier: account.twoFactorIdentifier,
    username: username,
  });
